...
눌러도 아무 일이 안 일어난다. 지금은 localStorage 에 있다.

알림도 서버가 없다. 브라우저가 관심공고와 마감일로 그 자리에서 계산한다.

    project/src/utils/favorites.js       관심공고 저장
    project/src/utils/notifications.js   D-7·D-3·D-1 계산

**카톡 알림(2단계)을 붙이면 이 전제가 깨진다.** 밤중에 서버가 보내야 하는데
localStorage 는 서버가 못 읽는다. 그때 Supabase 동기화를 `favorites.js` 안에
더한다 — 화면 쪽은 이 파일만 보고 있으므로 컴포넌트는 안 바뀐다.

카카오 로그인은 이미 있지만 알림에 못 쓴다. `api/auth_kakao.py` 가
`access_token` 으로 회원번호만 한 번 조회하고 버린다. 보내려면 `talk_message`
동의항목과 `refresh_token` 저장이 필요하다.
