...
| `requirements_unknown` | 문자열 | **판정하지 않고 무조건 "확인필요"** | `"공고문에서 자격 요건을 확인하지 못했습니다"` |

### `requirements_unknown` 은 왜 있나
...
공고를 구분할 방법이 없다. 첨부 중 기계로 읽히는 비율이 42% 뿐이고,
읽히더라도 "온라인판로 종합지원 선정 기업" 처럼 우리 프로필로는 판정할
수 없는 조건이 있다.

구분이 안 되면 안전한 쪽으로 통일한다. 사용자에게 "확인해보세요" 라고
말하는 건 손해가 없지만, 대상이 아닌 사람에게 "신청가능" 이라고 말하는
건 헛걸음을 시킨다.
