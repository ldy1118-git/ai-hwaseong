...
| `requirements_unknown` | 문자열 | **판정하지 않고 무조건 "확인필요"** | `"중소 제조업체(대형 가전ㆍ가구 제외 전 품목)"` |
| `requirements_open` | 문자열 | **판정하지 않고 무조건 "충족"** | `"소상공인이라면 누구나"` |

### `requirements_unknown` / `requirements_open` 은 왜 있나

요건을 하나도 못 뽑았을 때 조건을 아예 안 걸면, 매칭 결과가 **"조건 없음
= 신청가능 100점"** 이 되어 목록 맨 위로 올라간다. 실제로 25건 중 17건이
그렇게 떠 있었다.

그래서 한동안은 못 뽑은 걸 전부 `requirements_unknown` 으로 깔았다.
요건이 진짜 없는 공고(전 소상공인 대상)와, 요건이 있는데 우리가 못 읽은
공고를 구분할 방법이 없었기 때문이다. 안전하긴 했지만 **27건 중 19건이
프로필과 무관하게 확인필요**로 나왔다. 열에 일곱이 "모르겠습니다"면
매칭 서비스라고 하기 어렵다.

이제 구분한다. 기업마당 API 요약(`bsnsSumryCn`)은 형식이 고정돼 있다.

```
<사업 설명>  ☞  <지원대상>  ☞  <지원내용>
```

첫 `☞` 뒤에 지원대상이 그대로 적혀 있다. 27건 전부 이 형식이었다.

| 지원대상 표기 | 키 | 결과 |
|---|---|---|
| `소상공인이라면 누구나` | `requirements_open` | 충족 |
| `「소상공인기본법」 제2조에 따른 소상공인` | `requirements_open` | 충족 |
| `중소 제조업체(대형 가전ㆍ가구 제외 전 품목)` | `requirements_unknown` | 확인필요 |
| `온라인판로 종합지원을 받은 기업` | `requirements_unknown` | 확인필요 |

가르는 건 `extract.classify_target()`. **애매하면 unknown 쪽으로 둔다.**
놓치면 신청가능이어야 할 게 확인필요로 남을 뿐이지만, 잘못 통과시키면
대상이 아닌 사장님을 헛걸음시킨다. 그래서 아래는 전부 unknown 이다.

- 업종을 한정한다 (제조·섬유·화학·공장…)
- 기존 사업 참여 이력을 요구한다 (지원을 받은·선정된·참여한)
- 스스로 "자세한 지원대상 공고문 참조" 라고 말한다
- 항목이 `①②③` 로 나뉜다 — 조건이 더 붙어 있다
- 60자를 넘는다
- **제목에 화성·경기 아닌 지역명이 있다**

마지막 항목이 중요하다. 소담스퀘어 광주·강원·전주·경북 4건은 지원대상에
"소상공인" 이라고만 적혀 있지만 실제로는 그 지역 시설을 쓰는 사업이다.
화성시 사장님에게 신청가능으로 띄우면 헛걸음이다.

값에 넣은 문장이 화면에 그대로 나가므로 사용자가 읽을 말로 쓸 것.
