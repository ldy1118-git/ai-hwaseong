...
달력 칸을 누르면 그 날의 세무일정·공고·메모가 **달력 위 창**으로 뜬다.
달력 아래에는 아무것도 안 그린다 — 아래에 폈더니 날짜를 누를 때마다
달력이 위아래로 움직였고, 목록을 보려고 화면을 내리면 어느 날을 고른
건지 안 보였다.

오른쪽 목록에서 날짜를 누르면 달력이 그 달로 넘어가 그 칸을 편다.
**이때는 창을 안 띄운다** — 달력을 가리면 「어디인지 보려고」 누른 뜻이
사라진다. 그래서 고른 칸(`selKey`)과 창을 연 상태(`openDay`)가 따로 있다.
