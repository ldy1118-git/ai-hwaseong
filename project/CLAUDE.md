...
`llmLoading` 을 **켜는 곳과 끄는 곳이 갈라져 있어서**였다.
...
써버리고, 정작 핀이 도착해 로딩을 켠 뒤로는 아무도 끄지 않는다.

**켜는 effect 를 아예 없앴다.** 조건을 하나씩 채워 넣는 것으로는 또 새는
길이 나온다 — 한 값을 둘이 나눠 쓰는 모양 자체가 원인이다. 지금 `llmLoading`
을 쓰는 곳은 `handleAskMaida` 뿐이고, 켜면 `finally` 에서 반드시 끈다.
화면에 보이는 「분석 중」은 상태가 아니라 계산값이다(`analyzing`) — 추천을
받아오는 동안까지 이어서 보여주되, 계산값이라 잠길 수가 없다.

**같은 값을 두 effect 가 나눠 쓰게 두지 말 것.** 이 화면에서 두 번 물렸다.
...
자동 분석은 **핀이 도착하고, 그 자리의 상권 자료까지 온 뒤에** 돈다.
핀이 오면 지도가 1위 자리로 옮겨가는데 그 자리 자료는 300ms 디바운스 뒤에
오므로, 안 기다리면 **옛 자리의 수치로 새 자리를 설명한다.** `apiData` 에
어느 좌표의 것인지(`_at`)를 적어두고 맞을 때만 돈다. 추천을 새로 시작할
때마다 깃발을 다시 세운다.
