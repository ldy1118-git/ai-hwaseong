...
# 카톡 알림은 여기서 안 보낸다.
#
# 예전에는 공고를 받자마자 이어서 보냈다. 그러면 **새벽 6시에 카톡이 울린다.**
# 그건 알림이 아니라 민폐다. 사장님이 고른 시각·요일에 보내려면 발송이
# 하루 한 번이 아니라 매시 돌아야 해서, 아예 갈랐다.
#
#     06:11   이 스크립트 — 공고를 받아서 커밋·푸시
#     매시 정각 ~/bin/mars-kakao.sh — 지금이 제 시각인 사람에게만 발송
#
# 발송 쪽은 scripts/notify_kakao.py 의 due_now() 가 판단한다.
